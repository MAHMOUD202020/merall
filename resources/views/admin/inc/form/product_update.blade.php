<form  id="form_create_post" method="post" action="{{route('admin.product.fullUpdate' , $product->id)}}" enctype="multipart/form-data">
  @csrf
    <div class="form-row mb-4">

        <div class="form-group col-md-6">
            <label for="name">اسم المنتج</label>
            <input name="name" type="text" class="form-control @error('name') is-invalid @enderror" id="name" placeholder="الاسم الظاهر في الموقع" value="{{old('name') ? old('name') : $product->name}}" required>
            @error('name')<span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>@enderror
        </div>
        <div class="form-group col-md-6">
            <label for="slug">ال slug</label>
            <input name="slug" type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" placeholder="الاسم الظاهر في  ال url" value="{{old('slug') ? old('slug') : $product->slug }}">
            @error('slug')<span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>@enderror
        </div>

        <div class="form-group col-md-6">
            <label for="price">السعر</label>
            <input name="price" type="number" class="form-control @error('price') is-invalid @enderror" id="price" placeholder="سعر المنتج" value="{{old('price') ? old('price') : $product->price}}" step="any" required>
        ...
        <div class="form-group col-md-6">
            ...
            <input name="discount" type="number" class="form-control @error('discount') is-invalid @enderror" id="discount" placeholder="سعر المنتج بعد اضافة الخصم علية" value="{{old('discount') ? old('discount') : $product->discount}}" step="any">
        ...
        <div class="form-group col-md-6">
            ...
            <select name="sizeChart" id="sizeChart" class="form-control">
                ...
                @foreach($sizeCharts as $sizeChart)
                    <option {{(old('sizeChart') ? old('sizeChart') : $sizeChart->id) == $product->sizeChart_id ? 'selected' : ''}} value="{{$sizeChart->id}}">
                        {{$sizeChart->name}}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group col-12  @error('sizes') border border-danger @enderror">
            ...
            <div class="widget cover-size">
                @php($oldSizes = old('sizes') ? old('sizes') : $product->sizes->pluck('id')->all())
        ...
        <div class="form-group col-12  @error('cats') border border-danger @enderror">
            ...
            <div class="widget cover-cats">
                @php($oldCats = old('cats') ? old('cats') : $product->cats->pluck('id')->all())
            ...
            <div id="tooltip" class="row layout-spacing">
                <div class="col-lg-12">
                    <div class="statbox widget box box-shadow">
                        ...
                        <textarea id="mytextarea" name="description">{{old('description') ? old('description') : $product->description}}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="statbox widget box box-shadow @error('img') border border-danger @enderror">
                <div class="widget-header">
                    <div class="row">
                        ...
                        <div class="col-12 text-center">
                            <img src="{{asset("assets/web/images/products/min/small_$product->img")}}" alt="{{$product->alt}}" class="img-fluid border-info" width="200">
                        </div>
                    </div>
                </div>
                <div class="widget-content widget-content-area">
                    ...
                    <div class="form-group ">
                        ...
                        <input name="alt" type="text" class="form-control @error('alt') is-invalid @enderror" id="alt" placeholder="الاسم المعبر عن الصورة في حالة تركة فارغا سوف يتم تعين اسم المنتج  كا قيمة مدخلة لهاذا الحقل" value="{{old('alt') ? old('alt') : $product->alt}}">
        ...
        <div id="fuMultipleFile" class="col-lg-12 layout-spacing">
            <div class="statbox widget box box-shadow  @error('images') border border-danger @enderror">
                    ...
                    <div class="row">
                        <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                            ...
                            <div class="col-6 col-sm-4 col-md-3 col-lg-2 text-center mb-2">
                                <img src="{{asset("assets/web/images/products/gallery/$product->slug/small_$img->src")}}" alt="{{$img->alt}}" style="max-width: 100%">
                                <input type="hidden" value="{{$img->id}}" name="oldImg[]">
                                <a class="btn btn-danger btn-delete-img">حذف الصورة</a>
                            </div>
                            @endforeach
                    </div>
                </div>
                ...
                <div class="form-group ">
                    ...
                    @php($alt_images = $product->images->first()? $product->images->first()->alt : '')
                    <input name="alt_images" type="text" class="form-control @error('alt_images') is-invalid @enderror" id="alt_images" placeholder="الاسم المعبر عن الصورة في حالة تركة فارغا سوف يتم تعين اسم المنتج  كا قيمة مدخلة لهاذا الحقل" value="{{old('alt_images') ? old('alt_images') : $alt_images}}">
            ...
            <div class="widget">
                ...
                <div class=" mt-3 mb-3">
                    <div class="custom-control custom-checkbox mb-4">
                        @php($premium = old('premium') ? old('premium') : $product->premium)
                        <input name="premium" type="checkbox" class="custom-control-input" id="premium" {{$premium == 1 ? 'checked' : ''}}>
                        <label class="custom-control-label" for="premium">تميز المنتج</label>
                    </div>
                    <div class="custom-control custom-checkbox">
                        @php($available = old('available') ? old('available') : $product->available)
                        <input name="available" type="checkbox" class="custom-control-input" id="available" {{$available == 0 ? 'checked' : ''}}>
            ...
            <div class="widget  cover-colors">
                ...
                <div class="row mb-4 mt-3">
                    ...
                    <div class="col-sm-9 col-12">
                        ...
                        @php($oldColors = old('$colors') ? old('$colors') : $colors_checked->pluck('id')->all())
                        ...
                        <div class="tab-content" id="v-line-pills-tabContent">
                            <div class="tab-pane fade show active" id="v-line-pills-home" role="tabpanel" aria-labelledby="v-line-pills-home-tab">
                                <div class=" mt-3 mb-3" id="cover_all_colors">
                                    @foreach($colors_checked->where('custom' , 0) as $color)
                                        <div class="custom-control custom-checkbox mb-4">
                                            <input  value="{{$color->id}}" name="colors[]" type="checkbox" class="custom-control-input" id="{{$color->slug}}" {{ in_array($color->id , $oldColors) ? 'checked' : ''}}>
                            ...
                            <div class="tab-pane fade" id="v-line-pills-profile" role="tabpanel" aria-labelledby="v-line-pills-profile-tab">
                                <div class=" mt-3 mb-3" id="cover_custom_colors">
                                    @foreach($colors_checked->where('custom' , 1) as $color)
                                        <div class="custom-control custom-checkbox mb-4">
                                            <input  value="{{$color->id}}" name="colors[]" type="checkbox" class="custom-control-input" id="{{$color->slug}}" {{ in_array($color->id , $oldColors) ? 'checked' : ''}}>
                                            <label class="custom-control-label" for="{{$color->slug}}">
                                                {{$color->name}}
        ...
        <div class="cover_seo row">
            ...
            <div class="form-group col-12">
                ...
                <textarea name="meta_description" type="number" class="form-control  @error('meta_description') is-invalid @enderror" id="meta_description"   maxlength="255" rows="4">{{old('meta_description') ? old('meta_description') : $product->meta_description}}</textarea>
            ...
            <div class="form-group col-12">
                ...
                <textarea name="seo_title" type="number" class="form-control  @error('seo_title') is-invalid @enderror" id="seo_title"   maxlength="255" rows="4">{{old('seo_title') ? old('seo_title') : $product->seo_title}}</textarea>
            ...
            <div class="form-group col-12">
                ...
                <textarea name="keyword_tag" type="number" class="form-control  @error('keyword_tag') is-invalid @enderror" id="keyword_tag"   maxlength="255" rows="4">{{old('keyword_tag') ? old('keyword_tag') : $product->keyword_tag}}</textarea>
                @error('keyword_tag')<span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>@enderror
            </div>
        </div>

    </div>
    <button type="submit" class="btn btn-primary mt-3">تعديل المنتج </button>
</form>
