...
@section('breadcrumb')
    ...
    <li><a href="{{route('admin.order.index')}}">الطلبات</a></li>
    <li>تفاصيل الطلب</li>
        ...
        <div class="container">
            <div class="pt-account-layout">
                ...
                @if($order->status == 0)
                    <form  method="post">
                        @csrf
                        <button formaction="{{route('admin.order.replaceStatus' , [1 , $order->id])}}" type="submit" value="0" class="btn btn-success">الموافقة علي الطلب</button>
                        <button formaction="{{route('admin.order.replaceStatus' , [4 , $order->id])}}" type="submit" value="4" class="btn btn-success">رفض الطلب </button>
                    </form>
                @endif
                @include('admin.inc.message_success')

                <div class="pt-wrapper">
                    <a href="{{route('admin.order.index')}}" class="btn-link btn-lg pt-link-back">
                        <div class="pt-icon">
                            <svg width="24" height="24" viewBox="0 0 24 24">
                                <use xlink:href="#icon-arrow_large_left"></use>
                            </svg>
                        </div>
                        <span class="pt-text">العودة الي عرض جميع الطلبات</span>
                    </a>
                </div>
