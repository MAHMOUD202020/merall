...
function myDataTableColumns($columns) {
    ...
    if (tdOneVal !== 'none'){
        ...
        columnsData['tdOne'] = [];
        columnsData['tdOne'][0] =$('<td data-type="tdOne" data-name="tdOne" class="td-one notPrint notExport notEdit"><i class="fas fa-print print-single-row"></i><input type="checkbox" name="tdSelect[]" value="" class="td-select"></td>');
    ...
    if ($columns['btn']){
        ...
        $.each($columns['btn'] ,function ($btnName , btnAttr) {
            ...
            if ($btnName === 'edit') {
                ...
                columnsData['btn'][$btnName]['btnActionSingle'] = $('<a href="" class="btn btn-success dataEdit"> ' + langBtn['edit'] + '<i class="fas fa-edit fa-1x"></i> </a>')
                ...
                columnsData['btn'][$btnName]['btnActionSingle']   = $( '<a href="" class="btn btn-outline-danger dataDelete"> ' + langBtn['delete'] + '<i class="fas fa-trash-alt"></i> </a>')
                ...
                columnsData['btn'][$btnName]['btnActionSingle'] = $('<a target="_blank" href="" class="btn  btn-info '+$btnName+'"> ' + textSingle + ' ' + iconSingle+ '</a>')

                if (textSelected) {

                    columnsData['btn'][$btnName]['btnActionSelected'] = $('<a href="" class="btn  btn-info ' + $btnName + '"> ' + textSelected + ' ' + iconSelected + '</a>')
                }
            }
                ...
                responseData.forEach(function (row) {
                    ...
                    $.each(columnsData , function (name , columnContent) {
                        ...
                        if (!$.isArray(row[columnVal[0]])) {
                            ...
                            columnVal = columnVal[1] !== undefined
                                ? row[columnVal[0]][columnVal[1]]
                                : row[columnVal[0]]
                    ...
                    newRow.push('<tr>'+columns.toString()+'</tr>')
...
$(function () {
    ...
    let
        ...
        aliasName = [];
        elementModel = {
            'textarea' : $("<div class='form-group'><label></label><textarea name='' class='form-control' rows='6'></textarea><span class='invalid-feedback' user='alert'><strong></strong></span></div>"),
            'select'   : $("<div class='form-group'><label></label><select name='' class='custom-select custom-select-md mb-3'></select></div>"),
            'selectM'  : $("<div class='form-group'><label></label><select multiple name='' class='custom-select custom-select-md mb-3'></select></div>"),
            'normal'   : $('<div class="form-group "><label></label><input  type="" name="" class="form-control" value="">'),
            ...
            if (!isNotEdit && tdName !== 'tdOne' && tdName !== 'btn') {
                ...
                if (type === "textarea") {
                    ...
                    if (optionSelect[tdName][1] ==='%%') {
                        ...
                        $.each(JSON.parse(optionSelect[tdName][0]), function (key, val) {
                            if (alias){
                    ...
                    if (optionSelect[tdName][1] ==='%%') {
                        ...
                        $.each(JSON.parse(optionSelect[tdName][0]), function (key, val) {
                            ...
                            selected = dataValInputTrim.includes(val) ? 'selected' : '';
                            option.push('<option value="' + key + '"' + selected + '>' + val + '</option>')
                        })
        ...
        $.ajax({
            ...
            error:function (xhr) {
                ...
                $.each(xhr.responseJSON.errors , function (key , error) {
                    ...
                    $('.modalAction input.myDataTable-'+key).addClass('is-invalid').after(htmlError[0].outerHTML)
                })
            }
        })
